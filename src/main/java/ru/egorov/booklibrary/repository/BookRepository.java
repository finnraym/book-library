...
public interface BookRepository extends JpaRepository<Book, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT b.id, b.title, b.number_of_pages, b.year_of_issue FROM develop.book b " +
            "LEFT JOIN develop.book_author ba ON b.id = ba.book_id " +
            "WHERE ba.author_id=:authorId")
    List<Book> findAllByAuthorId(@Param("authorId") Long authorId);

    @Query(nativeQuery = true, value = "SELECT b.id, b.title, b.number_of_pages, b.year_of_issue FROM develop.book b " +
            "LEFT JOIN develop.book_genre bg ON b.id = bg.book_id " +
